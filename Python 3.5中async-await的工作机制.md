...
## 结论
总而言之，`async`和`await`出现的目的就是协程，顺便支持的可等待对象，和把普通生成器转换成协程。所有这些都是为了实现并发，来提升Python中的异步编程体验。相比使用多线程的编程体验，协程功能强大并且更为易用--一个完整的异步编程的示例，只用了包括注释在内的不到100行代码--兼具良好的适用性和效率(curio的FAQ里说它的运行速度比`twisted`快30-40%，但比`gevent`慢10-15%，别忘了，在Python2+版本中，Twisted用的内存更少而且调试比Go简单，你可以想象这种对比有多么巨大的意义！)。能在Python 3中看到`async/await`的引入，我非常高兴，并且期待Python社区接纳这个新成员，希望有更多的库和框架可以支持`async/await`语法，让所有的Python开发者都可以从异步编程中受益。
